...
public class DijkstraUndirectedSP {
     ...
     */
    public DijkstraUndirectedSP(GrafoNoDirigido G, int s) {
    	for (Object arc: G.darArcos()) {
        	Arco act = (Arco) arc;
            if (act.darPesoHaversine()< 0)
                throw new IllegalArgumentException("edge " + act + " has negative weight");
        }
        ...
        while (!pq.isEmpty()) {
            int v = pq.delMin();
            for (Object e : G.getVertex(v).adj())
                {
            	Arco x = (Arco) e;
            	relax(x, v);
                }
    ...
    private void relax(Arco e, int v) {
        int w = e.other(v);
        if (distTo[w] > distTo[v] + e.darPesoHaversine()) {
            distTo[w] = distTo[v] + e.darPesoHaversine();
            edgeTo[w] = e;
